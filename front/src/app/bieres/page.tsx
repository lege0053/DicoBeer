    ...
    async function fetchBeers() {
      try {
        const response = await fetch(
          "http://localhost:9000/api/beer/getAllBeers"
        );
        if (!response.ok) {
          throw new Error("Failed to fetch data");
        }
        const data = await response.json();
        setBeers(data.data[0]);
      } catch (error) {
          ...
          <li key={beer._id} className=" p-2 mb-2">
            <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow">
              ...
              <div className="px-5 pb-5">
                ...
                <p className="mb-2">
                  <span className="font-semibold">Id:</span> {beer._id}
                </p>
                <p className="mb-2">
                ...
                <p className="mb-2">
                  <span className="font-semibold">Degré d'alcool:</span>{" "}
                  {beer.degree + ""}%
                </p>
                <p className="mb-2">
